...
## Script
__Process_land_cover_map.r__. This script loads a country specific land use map, aggregates it to SIMU level and saves the information in GDX format. For the script to work, land cover data need to be presented as a raster and have a smaller resolution than the SIMU map. The following procedure is used to aggregate the land cover data:

1. Load the global SIMU 5 armin map and create a raster with grid cell area size.
2. Ensure that the country specific land cover map has the same projection as the SIMU map.
3. Create a map layer for each land use with grid cell area size.
4. Overlay each land cover layer with the SIMU map and calculate the total area of each land cover class per SIMU.
5. Calculate the relative area shares of each land cover class per SIMU.
6. Save the data in GDX format.



globe for IPCC reporting.is used for most land use maps and in most cases will also be used in the In most cases the classification of the new maps will have The researcher needs to aggegate the country specific land use maps to the same classes.

```{r Tab_1}
Tab_LC <- read_csv(file.path(root, "Data/Land_cover_class.csv"))
pander(Tab_LC)
```
###### Source: @Haack2015
