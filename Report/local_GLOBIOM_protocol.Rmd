...
- https://www.countrystat.org/, a website of the FAO with subnational agricultural statistics.
...
The abbreviations (as, hs and lc) are used in the names of the mappings and files to refer to the source.
...
NB: CHECK if we want to use GAUL (FAO) or GADM subnational regions.
...
### Administrative areas: iso3c2ADM_reg
...
### Crops: 
Map all crops that are produced in the country to one of the crop codes that are used in GLOBIOM. In some cases this is straigtforward, while in other cases it requires some thinking and creativity. Many tropical countries produce a large variety of beans (e.g. ordinary beans, ground beans, etc), which are all mapped to Beans, dry (BeaD). Similarly, cow peas and pigeon peas are typically mapped to chick peas [@check].
...
- Analyse which are the most important crops in the country measured by area and compare with FAOSTAT.
- Analyse whicha re the most important crops per administrative zone level
