...
This is a script to allocate land information for ZMB: min slack model
...
$set gdxOutput  H:\MyDocuments\Projects\Global-to-local-GLOBIOM\Model\ZMB\Data\min_slack_ZMB_2000.gdx
...
*    adm_slack(k,s,sign) Slack variable for adm area
    sum_slack           Weighted sum of abs(slacks)
...
equations
    obj_min_slack   Objective function: minimize weighted sum of abs(slacks)
...
*
* sum of slacks objective function
*

$ontext
* total slacks including for adm_area
* small adm_area: large penalty
parameter slackweights(k,s) 'if small deptot we like to have a large weight';
slackweights(k,s)$adm_area(k,s) = 1/adm_area(k,s);
display slackweights;

obj_min_slack.. sum_slack =e=sum(m$adm_area(m),
                   slackweights(m)*(adm_slack(m,'plus')+adm_slack(m,'minus')))
                   + 1e6*sum(i,lc_slack(i))
                   + 1e6*sum(i,ir_slack(i));
$offtext

* Total slacks excluding adm_area
obj_min_slack.. sum_slack =e= sum(i,lc_slack(i))+ sum(i,ir_slack(i));
...
* Model
model min_slack   /obj_min_slack, sum_one, grid_cover, ir_cover, adm_stat, det_alloc/;
*model min_slack   /obj_min_slack, sum_one, grid_cover, ir_cover, det_alloc/;
...
min_slack.holdfixed=1;

* Solve model
solve min_slack using lp minimizing sum_slack;

report('mstat') = min_slack.modelstat;
report('sstat') = min_slack.solvestat;
report('iterusd') = eps+min_slack.iterusd;
report('resusd') = eps+min_slack.resusd;
report('mrun')  = 1;
