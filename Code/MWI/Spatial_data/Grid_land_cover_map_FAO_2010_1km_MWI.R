...
### LOAD COUNTRY GRID
grid <- readRDS(file.path(dataPath, "Data/MWI/Processed/Maps/grid_30sec_MWI.rds"))
...
extract_grid_f <- function(polyID, cover){
  ...
  if(all(is.na(df_poly$layer)) |  nrow(df_poly)==0) {
    df_poly <- data.frame(gridID = polyID, no_data = 0)
  } else {
    df_poly <- df_poly %>%
      setNames(c("ID", "class")) %>%
      na.omit() %>%
...
saveRDS(lc_sh_raw, file.path(dataPath, "Data/MWI/Processed\\Spatial_data/land_cover_shares_raw_FAO_30_sec_2010_MWI.rds"))
lc_sh_raw <- readRDS(file.path(dataPath, "Data/MWI/Processed\\Spatial_data/land_cover_shares_raw_FAO_30_sec_2010_MWI.rds"))
