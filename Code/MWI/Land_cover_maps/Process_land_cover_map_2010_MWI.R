...
### LOAD LAND COVER MAP 
# Load map
land_cover_map_raw <- raster(file.path(dataPath, "Raw\\MWI\\Land_cover_maps\\RCMRD\\Final Corrected Land Cover\\Final Corrected Land Cover\\Scheme 2\\malawi 2010 classification scheme2.img"))
land_cover_map_raw
...
saveRDS(land_cover_shares_raw, file.path(dataPath, "Processed\\MWI\\Land_cover_maps/land_cover_shares_2010_raw.rds"))
land_cover_shares_raw <- readRDS(file.path(dataPath, "Processed\\MWI\\Land_cover_maps/land_cover_shares_2010_raw.rds"))
...
plot(mask3)
plot(SIMU2country_poly_rp[SIMU2country_poly_rp$SimUID %in% annual_cropland,], add = T, border = "red")
rm(mask3)
