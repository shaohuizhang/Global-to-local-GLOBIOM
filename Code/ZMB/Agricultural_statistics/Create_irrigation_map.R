...
# Additional packages
p_load("XLConnect", "measurements", "nominatim", "plotKML")
...
### LOAD MAPS
# polygons of irrigated areas
zambia_sugar <- readOGR(file.path(dataPath, paste0("Data/", iso3c_sel, "/Raw/Agricultural_statistics/System/Zambia_sugar_2000_12.kml")))

# gmia
ir_map <- raster(file.path(dataPath, paste0("Data\\", iso3c_sel, "/Processed/Maps/gmia/gmia_30sec_", iso3c_sel, ".tif")), overwrite = T)


### PROCESSS ADDITIONAL IRRIGATION INFORMATION
# Sugarcane
area(zambia_sugar)/10000 # m2 to ha
...
leaflet() %>% 
  addTiles() %>% 
  addRasterImage(ir_map, opacity = 0.5)
