...
as_raw <- read_excel(file.path(dataPath, paste0("Data/", iso3c_sel, "/Raw/Agricultural_statistics/Other/National_statistics/ag_statistics_1987_2014.xlsx"))) %>%
  dplyr::select(-Regions, -`Regions RegionId`, -`Regions TERRID`, -`Regions TOPO_ID`, -`Crop forecast`, -`Crop forecast Name`,
                -`Crop forecast`, -`Crop forecast FULLNAME`, -Units, -Scale) %>%
  rename(adm1_as = `Regions Name`, description = `Crop forecast DESCRIPTION`) %>%
...
as_raw$short_name <- ifelse(grepl("Sun", as_raw$description), "sunf", as_raw$short_name)
as_raw$short_name <- ifelse(grepl("Groundnuts", as_raw$description), "grou", as_raw$short_name)
as_raw$short_name <- ifelse(grepl("Soya", as_raw$description), "soyb", as_raw$short_name)
as_raw$short_name <- ifelse(grepl("Wheat", as_raw$description), "whea", as_raw$short_name)
as_raw$short_name <- ifelse(grepl("Sweet", as_raw$description), "swpo", as_raw$short_name)
as_raw$short_name <- ifelse(grepl("Cassava", as_raw$description), "cass", as_raw$short_name)
as_raw$short_name <- ifelse(grepl("Sugar", as_raw$description), "sugc", as_raw$short_name)
as_raw$short_name <- ifelse(grepl("Mixed Bean", as_raw$description), "bean", as_raw$short_name)
as_raw$short_name <- ifelse(grepl("Cotton", as_raw$description), "cott", as_raw$short_name)
